...
function BlogCard({ post, lang, onOpen }: { post: SoroPost; lang: Lang; onOpen: () => void }) {
  const lc = (ru: string, et: string, en: string) => lang === 'ru' ? ru : lang === 'et' ? et : en
  const cover = post.cover_image || post.cover || post.image || ''
  const excerpt = post.excerpt || post.description || ''
  const date = post.published_at || post.created_at || ''
  const dateStr = date ? (() => {
    try {
      return new Date(date).toLocaleDateString(
        lang === 'ru' ? 'ru-RU' : lang === 'et' ? 'et-EE' : 'en-US',
  ...
  return (
    <article
      ...
      style={{ cursor: 'pointer', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', borderRadius: 18, transition: 'transform 0.2s, box-shadow 0.2s, border-color 0.2s' }}
    >
      {/* Cover image */}
      {cover && (
        <div style={{ width: '100%', height: 196, overflow: 'hidden', flexShrink: 0, background: 'var(--bg-card-hover)' }}>
          <img
            src={cover}
            alt={post.title || ''}
            loading="lazy"
      ...
      {/* No cover: accent top border */}
      {!cover && (
        <div style={{ height: 3, background: 'linear-gradient(90deg, var(--accent), var(--accent-2))', flexShrink: 0 }} />
      )}

      {/* Content */}
      <div style={{ padding: '24px 28px 28px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        {dateStr && (
          <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10, fontWeight: 500, letterSpacing: '0.02em' }}>
            {dateStr}
          </p>
        )}

        <h3 style={{
          fontFamily: 'var(--font-inter)', fontSize: 17, fontWeight: 700, lineHeight: 1.35,
          letterSpacing: '-0.01em', marginBottom: 10, color: 'var(--text)',
          display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden',
        }}>
          {post.title || lc('Без заголовка', 'Pealkirjata', 'Untitled')}
        </h3>
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 5, color: 'var(--accent)', fontSize: 13, fontWeight: 600 }}>
          {lc('Читать статью', 'Loe artiklit', 'Read article')} →
        </div>
      </div>
    </article>
  )
...
function BlogSection({ lang, onCTA }: { lang: Lang; onCTA: () => void }) {
  const lc = (ru: string, et: string, en: string) => lang === 'ru' ? ru : lang === 'et' ? et : en
  const [posts, setPosts] = useState<SoroPost[]>([])
  const [loading, setLoading] = useState(true)
  const [allVisible, setAllVisible] = useState(false)
  const [activePost, setActivePost] = useState<SoroPost | null>(null)
  ...
  const openPost = async (post: SoroPost) => {
    setActivePost(post)
    setPostBody('')
    const existing = post.body || post.content || post.html || ''
    if (existing) { setPostBody(existing); return }
    setLoadingPost(true)
    try {
      const url = post.id
        ? `/api/soro?id=${encodeURIComponent(post.id)}`
        : `/api/soro?slug=${encodeURIComponent(post.slug || '')}`
      const r = await fetch(url)
      const d = await r.json()
      if (d.ok && d.post) {
        setPostBody(d.post.body || d.post.content || d.post.html || d.post.excerpt || '')
      }
    } catch { /* no-op */ }
    setLoadingPost(false)
  }

  const closePost = () => { setActivePost(null); setPostBody('') }

  const cover = activePost ? (activePost.cover_image || activePost.cover || activePost.image || '') : ''
  const excerpt = activePost ? (activePost.excerpt || activePost.description || '') : ''
  const dateStr = activePost && (activePost.published_at || activePost.created_at) ? (() => {
    try {
      return new Date(activePost.published_at || activePost.created_at || '').toLocaleDateString(
        lang === 'ru' ? 'ru-RU' : lang === 'et' ? 'et-EE' : 'en-US',
  ...
  if (activePost) {
    return (
      <section id="blog" style={{ padding: '80px 24px 96px', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <button
            onClick={closePost}
            style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 40, padding: 0, fontFamily: 'var(--font-inter)' }}
          >
            ← {lc('Все статьи', 'Kõik artiklid', 'All articles')}
          </button>

          {cover && (
            <div style={{ width: '100%', height: 'clamp(200px, 40vw, 360px)', borderRadius: 16, overflow: 'hidden', marginBottom: 36 }}>
              <img src={cover} alt={activePost.title || ''} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            </div>
          )}

          {dateStr && <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 14, fontWeight: 500 }}>{dateStr}</p>}

          <h1 style={{ fontFamily: 'var(--font-inter)', fontSize: 'clamp(24px, 4vw, 36px)', fontWeight: 800, letterSpacing: '-0.02em', lineHeight: 1.2, marginBottom: 32 }}>
            {activePost.title}
          </h1>

          {/* Body */}
          <div style={{ fontSize: 16, lineHeight: 1.85, color: 'var(--text-soft)' }}>
            {loadingPost ? (
              ...
              <p style={{ color: 'var(--text-muted)' }}>{lc('Полный текст не загрузился.', 'Täistekst ei laadinud.', 'Full text could not be loaded.')}</p>
            )}
          </div>
          ...
          <div style={{
            ...
            <h3 style={{ fontFamily: 'var(--font-inter)', fontSize: 'clamp(20px, 3vw, 26px)', fontWeight: 800, marginBottom: 12, letterSpacing: '-0.02em' }}>
              {lc('Нужен сайт для вашего бизнеса?', 'Vajate oma ettevõttele veebilehte?', 'Need a website for your business?')}
            </h3>
            <p style={{ color: 'var(--text-muted)', marginBottom: 28, lineHeight: 1.7, maxWidth: 440, margin: '0 auto 28px' }}>
              {lc(
                'Обсудим ваш проект бесплатно. Чёткие сроки, фиксированная цена, результат за 7 дней.',
                'Arutame teie projekti tasuta. Selged tähtajad, fikseeritud hind, tulemus 7 päevaga.',
                'Free project consultation. Clear timeline, fixed price, results in 7 days.'
              )}
            </p>
            <button
              onClick={() => { closePost(); setTimeout(() => onCTA(), 120) }}
              className="btn-primary"
              style={{ padding: '14px 40px', fontSize: 15 }}
            >
              {lc('Оставить заявку →', 'Küsi pakkumist →', 'Get a Free Quote →')}
            </button>
          </div>
        </div>
      </section>
  ...
  // ── Cards grid ─────────────────────────────────────────────────────────────
  const visible = allVisible ? posts : posts.slice(0, 3)
      ...
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-inter)', fontSize: 'clamp(28px, 4vw, 42px)', fontWeight: 800, marginBottom: 16, letterSpacing: '-0.02em' }}>
            {lc('Полезные статьи', 'Kasulikud artiklid', 'Useful articles')}
          </h2>
          <p style={{ color: 'var(--text-muted)', fontSize: 16, lineHeight: 1.75, maxWidth: 560, margin: '0 auto' }}>
            {lc(
              'Советы по веб-дизайну, маркетингу и развитию бизнеса онлайн.',
              'Näpunäited veebidisaini, turunduse ja veebipõhise äri arendamise kohta.',
        ...
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '60px 0' }}>
            <div style={{ width: 36, height: 36, border: '3px solid var(--border-light)', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          </div>
        )}

        {/* Empty */}
        {!loading && posts.length === 0 && (
          <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '40px 0', fontSize: 15 }}>
            {lc('Статьи скоро появятся 📝', 'Artiklid ilmuvad varsti 📝', 'Articles coming soon 📝')}
          </p>
        )}
        ...
        {!loading && posts.length > 3 && (
          <div style={{ textAlign: 'center', marginTop: 48 }}>
            ...
            >
              {allVisible
                ? lc('Свернуть ↑', 'Peida ↑', 'Show less ↑')
                : lc(`Показать все статьи (${posts.length}) →`, `Kuva kõik artiklid (${posts.length}) →`, `Show all articles (${posts.length}) →`)}
            </button>
          </div>
        )}
      </div>
...
export default function Home() {
  ...
  if (project.useIframe) {
    ...
    window.open(project.link, '_blank', 'noopener,noreferrer')
        ...
        {/* ── BLOG ── */}
        <BlogSection lang={lang} onCTA={() => scrollTo('contact')} />
